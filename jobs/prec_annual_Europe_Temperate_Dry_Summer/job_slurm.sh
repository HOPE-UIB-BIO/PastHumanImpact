#!/bin/tcsh
#SBATCH -J prec_annual_Europe_Temperate_Dry_Summer # (job name)
...
data_list <-
  readr::read_rds(
    here::here(
      "LA/prec_annual_Europe_Temperate_Dry_Summer/data_to_fit.rds"
    )
  )
...
if (
  ...
  readr::write_rds(
    data_mod,
    file = here::here("LA/prec_annual_Europe_Temperate_Dry_Summer/mod.rds")
  )
}
# ----------------------------------------------------------#
