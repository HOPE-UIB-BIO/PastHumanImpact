#!/bin/tcsh
#SBATCH -J prec_annual_Asia_Cold_Without_dry_season # (job name)
...
data_list <-
  readr::read_rds(
    here::here(
      "LA/prec_annual_Asia_Cold_Without_dry_season/data_to_fit.rds"
    )
  )
...
if (
  ...
  readr::write_rds(
    data_mod,
    file = here::here("LA/prec_annual_Asia_Cold_Without_dry_season/mod.rds")
  )
}
# ----------------------------------------------------------#
