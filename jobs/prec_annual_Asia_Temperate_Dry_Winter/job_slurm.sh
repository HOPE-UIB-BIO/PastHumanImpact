#!/bin/tcsh
#SBATCH -J prec_annual_Asia_Temperate_Dry_Winter # (job name)
...
data_list <-
  readr::read_rds(
    here::here(
      "LA/prec_annual_Asia_Temperate_Dry_Winter/data_to_fit.rds"
    )
  )
...
if (
  ...
  readr::write_rds(
    data_mod,
    file = here::here("LA/prec_annual_Asia_Temperate_Dry_Winter/mod.rds")
  )
}
# ----------------------------------------------------------#
