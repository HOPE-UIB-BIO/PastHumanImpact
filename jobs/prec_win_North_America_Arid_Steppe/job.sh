#!/bin/tcsh
#SBATCH -J prec_win_North_America_Arid_Steppe # (job name)
...
data_list <-
  readr::read_rds(
    here::here(
      "LA/prec_win_North_America_Arid_Steppe/data_to_fit.rds"
    )
  )
...
if (
  ...
  readr::write_rds(
    data_mod,
    file = here::here("LA/prec_win_North_America_Arid_Steppe/mod.rds")
  )
}
# ----------------------------------------------------------#
