#!/bin/tcsh
#SBATCH -J temp_cold_Asia_Temperate_Without_dry_season # (job name)
...
data_list <-
  readr::read_rds(
    here::here(
      "LA/temp_cold_Asia_Temperate_Without_dry_season/data_to_fit.rds"
    )
  )
...
if (
  ...
  readr::write_rds(
    data_mod,
    file = here::here("LA/temp_cold_Asia_Temperate_Without_dry_season/mod.rds")
  )
}
# ----------------------------------------------------------#
