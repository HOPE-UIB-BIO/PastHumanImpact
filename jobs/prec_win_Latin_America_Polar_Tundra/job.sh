#!/bin/tcsh
#SBATCH -J prec_win_Latin_America_Polar_Tundra # (job name)
...
data_list <-
  readr::read_rds(
    here::here(
      "LA/prec_win_Latin_America_Polar_Tundra/data_to_fit.rds"
    )
  )
...
if (
  ...
  readr::write_rds(
    data_mod,
    file = here::here("LA/prec_win_Latin_America_Polar_Tundra/mod.rds")
  )
}
# ----------------------------------------------------------#
