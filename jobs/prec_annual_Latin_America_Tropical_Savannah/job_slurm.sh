#!/bin/tcsh
#SBATCH -J prec_annual_Latin_America_Tropical_Savannah # (job name)
...
data_list <-
  readr::read_rds(
    here::here(
      "LA/prec_annual_Latin_America_Tropical_Savannah/data_to_fit.rds"
    )
  )
...
if (
  ...
  readr::write_rds(
    data_mod,
    file = here::here("LA/prec_annual_Latin_America_Tropical_Savannah/mod.rds")
  )
}
# ----------------------------------------------------------#
