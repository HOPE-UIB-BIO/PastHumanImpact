#!/bin/tcsh
#SBATCH -J prec_win_North_America_Temperate_Without_dry_season # (job name)
...
data_list <-
  readr::read_rds(
    here::here(
      "LA/prec_win_North_America_Temperate_Without_dry_season/data_to_fit.rds"
    )
  )
...
if (
  ...
  readr::write_rds(
    data_mod,
    file = here::here("LA/prec_win_North_America_Temperate_Without_dry_season/mod.rds")
  )
}
# ----------------------------------------------------------#
