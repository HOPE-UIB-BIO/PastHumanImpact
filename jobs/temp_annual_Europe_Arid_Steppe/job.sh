#!/bin/tcsh
#SBATCH -J temp_annual_Europe_Arid_Steppe # (job name)
...
data_list <-
  readr::read_rds(
    here::here(
      "LA/temp_annual_Europe_Arid_Steppe/data_to_fit.rds"
    )
  )
...
if (
  ...
  readr::write_rds(
    data_mod,
    file = here::here("LA/temp_annual_Europe_Arid_Steppe/mod.rds")
  )
}
# ----------------------------------------------------------#
