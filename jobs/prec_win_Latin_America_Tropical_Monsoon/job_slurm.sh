#!/bin/tcsh
#SBATCH -J prec_win_Latin_America_Tropical_Monsoon # (job name)
...
data_list <-
  readr::read_rds(
    here::here(
      "LA/prec_win_Latin_America_Tropical_Monsoon/data_to_fit.rds"
    )
  )
...
if (
  ...
  readr::write_rds(
    data_mod,
    file = here::here("LA/prec_win_Latin_America_Tropical_Monsoon/mod.rds")
  )
}
# ----------------------------------------------------------#
