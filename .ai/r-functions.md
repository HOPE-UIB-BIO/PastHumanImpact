...
Canonical guidance for authoring R functions, roxygen2 documentation, and function-level tests in this repository.
...
When updating functions or generating tests, stay contract-first and keep the change as small as possible.

- Write or fix roxygen before expanding tests.
- Add the smallest necessary input validation before writing tests that expect failures.
- Write tests from the documented contract, not from incidental implementation details.
- Do not perform style-only refactors while fixing function contracts or writing tests.
- Do not rewrite tidyverse code into base R, remove pipes, or otherwise change implementation style unless the task explicitly asks for it or the behavior fix depends on it.
- If a function does not have a clear contract, stop and document the gap instead of inferring behavior from the current implementation.

## Argument and Runtime Checks

Use different tools for different failures:

- `assertthat::assert_that()` for caller-supplied argument contracts: type, length, required columns, allowed values, and file existence.
- `cli::cli_abort()` for internal/data-content failures discovered after processing begins.
- `cli::cli_warn()` and `cli::cli_inform()` for warnings and progress messages.

Any function that prints progress or informational messages must accept a `verbose = TRUE` argument and guard messages with `if (isTRUE(verbose))`. Errors should not be silenced by `verbose`.

## Roxygen2

All new or substantially edited functions should have roxygen2 documentation immediately above the function definition.
...
Keep roxygen lines within the R code line limit. Documentation should state the intended behavior; tests should follow that intent rather than current bugs.
...
`R/00_Config_file.R` attaches project packages, sources functions under `R/functions/`, and defines project-wide constants. Source it before running any test so tests use the same function and package environment as the project.

The full-suite script entry point is `R/testing/run_all_tests.R`.
...
- Follow `.ai/r-coding.md` line width for all R code, including `testthat::test_that()` description strings.
- Do not use `$` for data-frame column access in test assertions. Prefer `dplyr::pull(data_frame, column_name)`.
- When the right-hand side is a function call, put it on the next line after `<-`.
- Use explicit namespaces in test files. At minimum, all `testthat` calls must be written as `testthat::...`.
...
- Prefer temporary directories and files for side effects (`tempdir()`, `tempfile()`).
- Avoid fixtures that require unavailable external data unless the test is explicitly marked as constrained.
...
4. At least one edge case (empty input, single-row input, boundary values, or `NA` handling) where relevant.
...
If a task establishes a different test location, document it in the relevant change and keep future tests consistent with that location.
...
For new function work, treat the relevant test run as a required final validation step before the change is considered complete.
...
For changes that touch shared helpers, pipeline contracts, or data structures, broaden validation according to `.ai/debugging.md`.

## Anonymous Functions

Inside `purrr` calls, use the tilde formula style with `.x`, `.y`, or `..1`/`..2` for `pmap()`. Do not use backslash lambdas inside `purrr` calls.
...
When `purrr::map()` calls are nested, bind the outer `.x` to a named object before entering the inner map so the code is unambiguous.
