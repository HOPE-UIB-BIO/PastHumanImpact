testthat::test_that("fit_hvarpart_models() maps fit_varhp over data_merge with response vars", {
  data_source <-
    data.frame(
      dataset_id = 1,
  ...
  old_get_varhp <-
    get("fit_varhp", envir = .GlobalEnv)

  on.exit(
    assign("fit_varhp", old_get_varhp, envir = .GlobalEnv),
    add = TRUE
  )
...
testthat::test_that("fit_hvarpart_models() uses distance column when provided", {
  data_source <-
    data.frame(
      dataset_id = 1,
      data_merge = I(list(data.frame(age = c(1, 2), spd = c(10, 20)))),
...
testthat::test_that("fit_hvarpart_models() validates required inputs", {
...
testthat::test_that("fit_hvarpart_models() can continue when one group fails", {
  data_source <-
    data.frame(
      dataset_id = c(1, 2),
      data_merge = I(
