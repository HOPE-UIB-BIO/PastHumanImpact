testthat::test_that("fit_pcoa() returns points and eigenvalues", {
  mat_m2 <-
    matrix(
      c(
        0, 0.5, 0.8,
...
testthat::test_that("fit_pcoa() accepts non-symmetric matrices", {
  mat_m2 <-
    matrix(
      c(
        0, 0.2, 0.7,
  ...
  testthat::expect_warning(
    res_pcoa <- fit_pcoa(data_m2 = mat_m2),
...
testthat::test_that("fit_pcoa() errors when distances include NA", {
  mat_m2 <-
    matrix(
      c(
        0, 0.5, NA,
...
testthat::test_that("fit_pcoa() validates matrix input", {
  testthat::expect_error(
    fit_pcoa(data_m2 = c(0, 1, 2)),
