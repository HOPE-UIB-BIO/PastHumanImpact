testthat::test_that("prepare_interpolated_model_data() reshapes interpolated values by dataset", {
  data_source <-
    data.frame(
  ...
  result <-
    prepare_interpolated_model_data(
...
testthat::test_that("prepare_interpolated_model_data() preserves expected columns in nested data", {
  data_source <-
    data.frame(
      data_to_fit = I(list(data.frame(
...
testthat::test_that("prepare_interpolated_model_data() interpolates linearly between points", {
  data_source <-
    data.frame(
      data_to_fit = I(list(data.frame(
        dataset_id = c(1, 1),
...
testthat::test_that("prepare_interpolated_model_data() validates data_to_fit column", {
  bad_source <-
    data.frame(
      dataset_id = 1,
      stringsAsFactors = FALSE
