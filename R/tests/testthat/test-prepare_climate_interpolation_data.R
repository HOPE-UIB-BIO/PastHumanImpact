testthat::test_that("prepare_climate_interpolation_data() unnests and nests by selected vars", {
  ...
  result <- prepare_climate_interpolation_data(
    data_source,
...
testthat::test_that("prepare_climate_interpolation_data() keeps expected rows per variable", {
  climate_df <- data.frame(
    age = c(100, 200, 300),
    time_id = c(1L, 2L, 3L),
  ...
  result <-
    prepare_climate_interpolation_data(
...
testthat::test_that("prepare_climate_interpolation_data() returns empty for unmatched sel_var", {
  climate_df <- data.frame(
    age = c(100, 200),
    time_id = c(1L, 2L),
    temp_annual = c(10, 11),
...
testthat::test_that("prepare_climate_interpolation_data() validates climate_data column", {
  bad_source <-
    data.frame(
      dataset_id = 1L,
      stringsAsFactors = FALSE
  ...
  testthat::expect_error(
    prepare_climate_interpolation_data(data_source = bad_source),
    regexp = "climate_data"
  )
})
