testthat::test_that("prepare_event_model_data pivots nested events to var groups", {
  data_source_events <-
    data.frame(
  ...
  result <-
    prepare_event_model_data(data_source_events)
...
testthat::test_that("prepare_event_model_data() drops NA values", {
  data_source_events <-
    data.frame(
      events_updated = I(list(data.frame(
        dataset_id = c(1, 1),
  ...
  result <- prepare_event_model_data(data_source_events)
...
testthat::test_that("prepare_event_model_data() returns expected var groups", {
  data_source_events <-
    data.frame(
      events_updated = I(list(data.frame(
        dataset_id = c(1, 1, 1),
...
testthat::test_that("prepare_event_model_data() validates events_updated column", {
  bad_source <-
    data.frame(
      dataset_id = 1,
      stringsAsFactors = FALSE
  ...
  testthat::expect_error(
    prepare_event_model_data(bad_source),
