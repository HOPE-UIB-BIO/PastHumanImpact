testthat::test_that(
  "select_spatial_thinning() is deterministic and respects minimum distance",
  {
    data_input <-
      tibble::tibble(
        dataset_id = 1:8,
    ...
    vec_distance_valid <-
      ...
      purrr::map_lgl(
        .f = ~ {
          ...
          mat_distance <-
            compute_spatial_distance_matrix(data_source = .x)
...
testthat::test_that(
  "select_spatial_thinning() validates distances and strata",
  {
    data_input <-
      tibble::tibble(
        dataset_id = 1:2,
