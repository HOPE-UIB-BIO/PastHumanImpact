testthat::test_that("prepare_metadata() adds age limits and climatezone labels", {
  data_assembly <-
    data.frame(
      dataset_id = 1,
      handle = "site-1",
  ...
  result <-
    prepare_metadata(data_assembly)
...
testthat::test_that("prepare_metadata() keeps selected metadata columns", {
  data_assembly <-
    data.frame(
      dataset_id = 2,
      handle = "site-2",
  ...
  result <- prepare_metadata(data_assembly)

  testthat::expect_true(all(c("dataset_id", "handle", "country", "region", "age_min", "age_max", "climatezone") %in% names(result)))
  testthat::expect_equal(nrow(result), 1L)
})

testthat::test_that("prepare_metadata() climatezone logic follows configured precedence", {
  data_assembly <-
    data.frame(
      dataset_id = c(1, 2, 3),
      handle = c("a", "b", "c"),
...
testthat::test_that("prepare_metadata() validates required columns", {
  data_assembly <-
    data.frame(
      dataset_id = 1,
      handle = "site-1",
  ...
  testthat::expect_error(
    prepare_metadata(data_assembly = data_assembly),
    regexp = "must contain a levels column"
  )
})

testthat::test_that("prepare_metadata() validates selected variables", {
  data_assembly <-
    data.frame(
      dataset_id = 1,
