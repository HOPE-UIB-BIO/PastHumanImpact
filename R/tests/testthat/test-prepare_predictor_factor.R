testthat::test_that("prepare_predictor_factor() constrains factor levels", {
  data_input <-
    data.frame(
  ...
  res_data <-
    prepare_predictor_factor(data_input)
...
testthat::test_that("prepare_predictor_factor() preserves NA values", {
  data_input <-
    data.frame(
...
testthat::test_that("prepare_predictor_factor() returns expected column count", {
  data_input <-
    data.frame(
...
testthat::test_that("prepare_predictor_factor() errors without predictor", {
  data_input <-
    data.frame(
      value = c(1, 2)
    )
  ...
  testthat::expect_error(
    prepare_predictor_factor(data_source = data_input),
