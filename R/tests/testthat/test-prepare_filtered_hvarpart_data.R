testthat::test_that("prepare_filtered_hvarpart_data removes Africa and private Latin America, applies age range", {
  ...
  result <- prepare_filtered_hvarpart_data(
    data_source,
    data_meta,
    age_from = 2000,
    age_to   = 8500,
...
testthat::test_that("prepare_filtered_hvarpart_data() keeps private Latin America when remove_private is FALSE", {
  data_meta <-
    data.frame(
      dataset_id = c(1L, 2L),
...
testthat::test_that("prepare_filtered_hvarpart_data() validates remove_private input", {
  data_meta <-
    data.frame(
      dataset_id = 1L,
      region = "Europe",
...
testthat::test_that("prepare_filtered_hvarpart_data() returns expected columns and nested structure", {
  data_meta <-
    data.frame(
      dataset_id = c(1L, 2L),
      region = c("Europe", "Latin America"),
