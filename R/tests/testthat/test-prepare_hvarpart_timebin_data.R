testthat::test_that("prepare_hvarpart_timebin_data() unnests, joins metadata, and returns n_samples count", {
...
testthat::test_that("prepare_hvarpart_timebin_data() output is a tibble", {
  data_source <- tibble::tibble(
    dataset_id = 1L,
    data_merge = list(
      data.frame(
...
testthat::test_that("prepare_hvarpart_timebin_data() groups correctly across multiple regions", {
  data_source <- tibble::tibble(
    dataset_id = c(1L, 2L),
    data_merge = list(
...
testthat::test_that("prepare_hvarpart_timebin_data() validates required columns", {
  data_source <-
    data.frame(
      dataset_id = 1L,
      stringsAsFactors = FALSE
