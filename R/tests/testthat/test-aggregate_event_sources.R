testthat::test_that("aggregate_event_sources() merges to nested events", {
  data_source_indices <-
    data.frame(
      dataset_id = c(1, 1),
      age = c(10, 20),
...
testthat::test_that("aggregate_event_sources() returns one row per dataset", {
  data_source_indices <-
    data.frame(
      dataset_id = c(1, 2),
      age = c(100, 100),
...
testthat::test_that("aggregate_event_sources() sets no_impact when both weak and strong are false", {
  data_source_indices <-
    data.frame(
      dataset_id = 10,
      age = c(100, 200),
...
testthat::test_that("aggregate_event_sources() validates required columns", {
  data_source_indices <-
    data.frame(
      dataset_id = 1,
      age = 100,
