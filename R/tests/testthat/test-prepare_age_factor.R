testthat::test_that("prepare_age_factor() converts years to kyr factor", {
  data_input <-
    data.frame(
  ...
  res_data <-
    prepare_age_factor(data_source = data_input)
...
testthat::test_that("prepare_age_factor() preserves non-age columns", {
  data_input <-
    data.frame(
      age = c(0, 500, 1000),
      value = c("x", "y", "z"),
...
testthat::test_that("prepare_age_factor() sets out-of-range ages to NA", {
  data_input <-
    data.frame(
...
testthat::test_that("prepare_age_factor() errors without required age column", {
  data_input <-
    data.frame(
      value = c(1, 2)
    )
