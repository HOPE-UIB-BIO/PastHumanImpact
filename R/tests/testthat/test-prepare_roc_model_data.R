testthat::test_that("prepare_roc_model_data() computes weights and nests roc data", {
  data_source_roc <-
    data.frame(
  ...
  result <-
    prepare_roc_model_data(data_source_roc)
...
testthat::test_that("prepare_roc_model_data() returns expected columns", {
  data_source_roc <-
    data.frame(
      PAP_roc = I(list(data.frame(
        dataset_id = c(2, 2),
  ...
  result <- prepare_roc_model_data(data_source_roc)
...
testthat::test_that("prepare_roc_model_data() drops NA ROC rows", {
  data_source_roc <-
    data.frame(
      PAP_roc = I(list(data.frame(
        dataset_id = c(1, 1, 1),
...
testthat::test_that("prepare_roc_model_data() validates PAP_roc column", {
  bad_source <-
    data.frame(
      dataset_id = 1,
      stringsAsFactors = FALSE
  ...
  testthat::expect_error(
    prepare_roc_model_data(bad_source),
    regexp = "PAP_roc"
  )
})