testthat::test_that("spatial plot keeps human-only values for both profiles", {
  ...
  data_importance <- tibble::tibble(
    ...
    has_negative_individual = rep(c(TRUE, FALSE, FALSE, FALSE), each = 2L),
  ...
  testthat::expect_s3_class(result$signed_full_range_plot, "ggplot")
  testthat::expect_identical(unique(result$record_values$predictor), "human")
