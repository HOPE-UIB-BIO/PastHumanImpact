testthat::test_that("prepare_spd_model_data() nests rounded long data by variable", {
  data_source_spd <-
    data.frame(
      dataset_id = 1,
  ...
  result <-
    prepare_spd_model_data(data_source_spd)
...
testthat::test_that("prepare_spd_model_data() returns expected var names", {
  data_source_spd <-
    data.frame(
      dataset_id = 2,
      spd = I(list(data.frame(
  ...
  result <- prepare_spd_model_data(data_source_spd)
...
testthat::test_that("prepare_spd_model_data() rounds to three decimals", {
  data_source_spd <-
    data.frame(
      dataset_id = 1,
      spd = I(list(data.frame(
...
testthat::test_that("prepare_spd_model_data() validates spd column", {
  bad_source <-
    data.frame(
      dataset_id = 1,
      stringsAsFactors = FALSE
  ...
  testthat::expect_error(
    prepare_spd_model_data(bad_source),
