testthat::test_that("aggregate_events() row-binds multiple event tables", {
  data_a <-
    data.frame(
      dataset_id = 1,
      age = 100,
  ...
  result <-
    aggregate_events(data_a, data_b)
...
testthat::test_that("aggregate_events() keeps all rows from all inputs", {
  ...
  result <- aggregate_events(data_a, data_b, data_c)

  testthat::expect_equal(nrow(result), 3L)
  testthat::expect_identical(dplyr::pull(result, dataset_id), c(1, 2, 3))
})

testthat::test_that("aggregate_events() handles empty inputs", {
  ...
  result <- aggregate_events(data_a, data_b)

  testthat::expect_equal(nrow(result), 1L)
  testthat::expect_identical(dplyr::pull(result, dataset_id), 5)
})

testthat::test_that("aggregate_events() validates input types", {
  data_a <-
    data.frame(dataset_id = 1, age = 100, weak = TRUE, strong = FALSE)

  testthat::expect_error(
    aggregate_events(data_a, "not_a_table"),
