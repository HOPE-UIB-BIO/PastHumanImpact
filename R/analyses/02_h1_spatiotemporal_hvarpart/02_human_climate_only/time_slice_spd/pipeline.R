...
#
#----------------------------------------------------------#
# Defines the time-slice spd h1 models target graph.
# Run with:
...
list(
  ...
  targets::tar_target(
    ...
    command = load_target_store_value(
      ...
      dplyr::filter(
        .data[["profile_id"]] ==
          "time_slice_spd_human_climate"
      )
  ),
  # Why: Compute temporal SPD once so downstream summaries reuse the same
  #   result.
  targets::tar_target(
  ...
  # Why: Prepare hvarpart temporal SPD importance so downstream targets share
  ...
  targets::tar_target(
    ...
    command = prepare_h1_result_records(
      data_components = data_hvarpart_temporal_spd_importance,
      ...
      configuration_hash = rlang::hash(
        list(h1_response_variables, h1_predictor_sets[["spd"]])
      )
    )
  )
)
