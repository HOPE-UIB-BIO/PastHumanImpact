...
vec_temporal_labels <-
  resolve_temporal_variable_label(vec_temporal_variables)
...
run_directory_setup(path_prediction_dir)
...
#----------------------------------------------------------#
# 2. Predict every fitted dataset -----
#----------------------------------------------------------#

list_dataset_predictions <-
  purrr::map(
    ...
    .progress = "Predicting dataset temporal trends"
  )
...
assertthat::assert_that(
  nrow(data_dataset_predictions) > 0L,
  all(
    data_dataset_predictions[["prediction_estimand"]] ==
      "dataset_specific"
  ),
...
data_dataset_observed <-
  ...
  dplyr::mutate(
    variable_label = factor(
      resolve_temporal_variable_label(variable),
...
data_dataset_raw <-
  prepare_raw_temporal_data(
    ...
    dataset_ids = unique(
      as.character(data_dataset_observed[["dataset_id"]])
    ),
    age_min = 500,
    age_max = 8500
  ) %>%
...
vec_dataset_ids <-
  intersect(
    intersect(
      unique(as.character(data_dataset_observed[["dataset_id"]])),
      unique(as.character(data_dataset_predictions[["dataset_id"]]))
    ),
    unique(as.character(data_dataset_raw[["dataset_id"]]))
  ) %>%
  sort()
...
list_raw_by_dataset <-
  split(
    data_dataset_raw,
    as.character(data_dataset_raw[["dataset_id"]])
  )
list_predictions_by_dataset <-
  split(
    data_dataset_predictions,
    as.character(data_dataset_predictions[["dataset_id"]])
  )
list_metadata_by_dataset <-
  split(
    data_dataset_metadata,
    as.character(data_dataset_metadata[["dataset_id"]])
  )
...
purrr::pwalk(
  ...
  .f = save_dataset_temporal_figure,
  ...
  dpi = 300,
  verbose = TRUE,
  .progress = "Saving dataset temporal figures"
)
