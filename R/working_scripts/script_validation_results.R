...
# inspect data
input_full_data <- data_spatial_vis %>%
  pivot_longer(Unique:Average.share, 
               names_to = "variance_partition", 
               values_to = "var" ) %>%
  mutate(perc = var/expl_var*100) 

input_full_data %>% 
  dplyr::filter(expl_var < 0)  %>% View()

#filter out models with negative total variation
# not comparable with the others
poor_model_dataset_ids <- input_full_data %>% 
  dplyr::filter(expl_var < 0) %>%
  pluck("dataset_id") %>% unique()

poor_model_dataset_ids

data_check <- output_spatial %>%
  dplyr::filter(dataset_id %in% as.character(poor_model_dataset_ids)) %>%
 inner_join(tar_read(data_meta) %>% 
                            dplyr::select(dataset_id, ecozone_koppen_5, region), 
                          by = "dataset_id")
data_check$varhp[[1]]

data_check$data_merge[[1]] %>% 
  dplyr::select(age:density_diversity) %>% 
  GGally::ggpairs()

data_for_mod <- data_check$data_merge[[3]]

mod <-  vegan::capscale(data_for_mod %>% 
                          dplyr::select(n0:density_diversity) ~ 1,
                        dist = "gower",
                        add = TRUE,
                        data = data_for_mod) 
...
sc1 <- vegan::scores(mod, display = "sites", choice = 1)

par(mfrow = c(1,2))
plot(sc1 ~ data_for_mod$age,type = "l" )
plot(data_for_mod$spd ~ data_for_mod$age, type = "l")


trans <- vegan::vegdist(data_for_mod %>% 
...
res <- mvpart::mvpart(data.matrix(trans) ~ age + spd,
                      xv = "1se",
                      xvmult = 500,
                      data = data.frame(data_for_mod))




input_full_data  %>%
  dplyr::filter(predictor == "human" & p_val < 0.05) %>% view()
