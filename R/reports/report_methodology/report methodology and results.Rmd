...
overview_table <- tab_gen %>% 
  ...
  mutate(Settings = c(rep("General",7), 
                       rep("Neotoma", 8), 
                       rep("Age-depth\n models", 9), 
                       rep("Site filtering", 12)))

overview_table %>% 
  flextable::as_grouped_data(groups = c("Settings"), 
                             columns = NULL) %>%
  flextable::flextable() %>%
  flextable::set_caption("Table 1: Selection of settings applied in FOSSILPOL") %>%
  flextable::add_footer_lines("Data download: 23-05-22") %>%
  flextable::autofit()
...
event_table %>%
  flextable::flextable() %>%
  flextable::set_caption("Table 2: Type of human events identified in pollen diagrams") %>%
  flextable::autofit()
...
The methodology for identifying past human activity in Latin America apply a different automated process to get past human events. An extensive literature review has been devoted to the detection of pollen types that are associated with human activity. The composition of indicator groups varies across the continent, regions, and countries. A computer algorithm has been created to assess each pollen record, this algorithm traverses the combinations of taxa that are recorded along with the specific indicator groups, and determined if the signal of a pollen indicator for human activity is weak or strong based on a defined threshold.  
...
```{r human indicator table for LA, eval=FALSE, include=FALSE}
...
human_indicator_table %>%
  ...
  flextable::flextable() %>%
  flextable::set_caption("Table 3: Human event indicators for Latin America") %>%
  flextable::autofit()

```
  

```{r human impact table for LA, eval=FALSE, include=FALSE}
...
human_impact_table %>%
  ...
  flextable::flextable() %>%
  flextable::set_caption("Table 4: Human event indices for Latin America") %>%
  flextable::autofit()
...
```{r figure of human events in LA}
...
```{r table of regional age cutoff}
...
age_cutoff_rc %>%
  flextable::flextable() %>%
  flextable::set_caption("Table 5: Minimum ages above which C14 data was removed (age_from) for different regions") %>%
  flextable::autofit()
...
```{r data preparation of the results}
...
# remove
dataset_spatial_vis <- data_spatial_vis %>%
...
```{r extract median values for the results}
...
```{r regional figure for North America}
...
```{r regional figure for Latin America}
...
Figure 2. The summary results for Latin America.   
<br>
<br>

```{r regional figure for Europe}
...
Figure 3: The summary results for Europe 
<br>
<br>

```{r regional figure for Asia}
...
Figure 4: The summary results for Asia.  
<br>
<br>


```{r regional figure for Oceania}
...
Explorative figures of temporal changes in procrustes sum-of-square (m2):

<br>

```{r figure of changes in procrustes sum-of-squares}
...
<!-- ```{r summary table for individual records, eval=FALSE, include=FALSE} -->

<!-- data_spatial_vis %>% -->
<!--  flextable::flextable() %>% -->
<!--  flextable::set_caption("Table 6: Summary results for individual records in the spatial analysis (within each record)") %>% -->
<!--   flextable::autofit() -->
...
<!-- ```{r summary table of the median results, eval=FALSE, include=FALSE} -->
...
<!--                 ends_with("median")) %>% -->
<!--   flextable::flextable() %>% -->
<!--  flextable::set_caption("Table 7: The median adjusted r2 summary for the spatial analysis calculated by grouping ecozones & regions") %>% -->
<!--   flextable::autofit() -->
...
<!-- ```{r summary table for the temporal analysis, eval=FALSE, include=FALSE} -->

<!--  data_temporal_vis %>% -->
<!--  flextable::flextable() %>% -->
<!--  flextable::set_caption("Table 8: The summary for individual records in the temporal analysis (within each time step in different regions)") %>% -->
<!--   flextable::autofit() -->
...
<!-- ```{r summary table for the median results, eval=FALSE, include=FALSE} -->
...
<!--                 ends_with("median")) %>% -->
<!--   flextable::flextable() %>% -->
<!--   flextable::set_caption("Table 9: The median adjusted r2 summary for the median analysis") %>% -->
<!--   flextable::autofit() -->
...
Our overall results showed that humans have minor effect on changes in ecosystem properties over time and the conclusion is that that past humans have not significantly alter fundamental ecosystem properties across regions. 

To explore our results we did several explorative analysis without constraining the data. This complied a lot of results not shown here. Basically that there is variation in ecosystem properties, and that there is weak correlation with time and past human presence, but this vary in different records, ecozones, and regions. 

We also explore how well age and/ or past humans are to separate variation in the data using MRTs. The idea wasto use cascading MRTs to disentagle interactions with age, if we first make a model with age, and then make new models with subsequent MRTs of resulting leafs using climate and spd as predictors. This approach (not shown here) did not work well. 

In summary, age explains very little of the variation of ecosystem properties on ecozones and/or continent levels. Comparing it with spd, spd actually explained the partitions slightly better, but prediction power was very bad. Adding the climatic variables, the models improved very much indicating the different climate variables are better predictors. However, the problem with this analysis is the time dependency between assemblage in the individual records. We need to "remove" the this variation on the local scale to compare it on a larger scale. 

The only solution to this I could think of is to make a new model using partial distance-based RDA. For each records, we run a model that capture the variation in each local record only constrained by spd, and at the same time remove the variation explained by time and climate. 

Extracting the adjusted r2 will say indicate if this is a good fit. Since the main focus is on humans and not climate, I did this only for the spds as the hierarchical variation partitioning already show that climate is a better predictor. This analysis can then be used as zooming in on the humans and provide some addition "proof" to back up the conclusion that past human presence did not lead to widespread changes in fundamental ecosystem properties. 

For each record we extract the constrained scores (CAP1), and the adjusted r2 (also age and spd variable), and make a dataframe of the constrained scores that can be plotted against time. CAP1 represent only the variations in ecosystem properties determined by the presence of past humans in each record. The adjusted r2 is often negative ~ 0, indicating that human presence explains nothing of the variation in the ecosystem properties. I therefore make a classification of the adjusted r2 to say something about amount of human impact, and to add this to the summary of H1.  

This analysis is not a validation per se, it is complementary to the hierarchical variation partitioning where we use the same model, but with all predictors included. I checked the adjusted r2 from the unique constribution in hierachical variation partitioning, and the partial models show different results between the records, but the overall results is the same. 

In the end, the MRTs are then used to detect if there is a consistent pattern in CAP1 in records within the different regions, but only for those records where the adjusted r2 is above 0.1 (where past humans explains more than 10 % of the variation of ecosystsem properties).

This is done on all regions. 
...
```{r prepare data and run analysis}
...
# Variation in Ecosystem properties constrained by humans 
Constraining the scores of ecosystem properties will show ecosystem patterns corresponding with the patterns of spds (not shown here). The adjusted r2 indicate how well this variation explains the data. Here (TO BE DISCUSSED IF RELEVANT) the adjusted r2 is grouped into human impact of "no" ~ <= 0, "low" ~ > 0 & <= 0.1, "moderate" ~ > 0.1 & <= 0.3, "high" ~ >0.3 & <= 0.5 > and "very high" ~ > 0.5.  

The following figures show scores of changes in ecosystem properties for individual records that indicate past presence of human corresponded to more than 10% of variation in ecosystem properties.


```{r figure North America}
select_region <- "North America"
...
```{r figure Latin America}

select_region <- "Latin America"

fig_2 <- summarise_human_impact %>%
  summarise_human_impact %>%
   filter(adjr2 > 0.1) %>%
   dplyr::filter(region == select_region) %>%
   dplyr::select(dataset_id:sel_classification, CAP1, age) %>%
...
fig_4 <-
 summarise_human_impact %>%
  ...
  geom_line(aes(col = as.numeric(dataset_id)),alpha = 0.3, linewidth = 1) +
  scale_colour_gradient2(low = "yellow2", mid = "orange3", high = "darkgreen")+
  facet_wrap(~sel_classification, ncol = 2) +
  theme_bw()+
  theme(
    legend.position = "none"
  ) +
  labs(title = "Asia",
       y = "Scores of ecosystem properties constrained by spd",
       x = "Age ca. BP")
...
# Number of records showing different degree of human impact

The following figures show number of records and the proportion of records with the classification of human impact for the different regions and selected classification


```{r figure counts human impact North America}

fig_6 <- summarise_human_impact %>% 
  dplyr::select(region, 
                sel_classification, 
                human_impact, 
                n_impacts, 
...
```{r figure proportions human impact North America}


fig_7 <-  summarise_human_impact %>% 
  dplyr::select(region, 
                sel_classification, 
                human_impact, 
                n_impacts, 
  ...
  facet_wrap(~sel_classification) +
  theme(legend.position = "none") +
  labs(y = "proportion number of records", title = "North America")
...
```{r figure counts human impact Latin America}
...
```{r figure proportions human impact Latin America}
...
fig_9  <- 
  ...
  facet_wrap(~sel_classification) +
   theme(legend.position = "none") +
  labs(y = "proportion number of records", title = "Latin America")
...
```{r figure counts human impact Europe}
...
```{r figure proportions human impact Europe}


 fig_11 <- summarise_human_impact %>% 
  dplyr::select(region, 
                sel_classification, 
                human_impact, 
                n_impacts, 
  ...
  facet_wrap(~sel_classification) +
   theme(legend.position = "none") +
  labs(y = "proportion number of records",
  title = "Europe")
...
```{r figure counts human impact Asia}
...
```{r figure proportions human impact Asia}


fig_13 <-  summarise_human_impact %>% 
  dplyr::select(region, 
                sel_classification, 
                human_impact, 
                n_impacts, 
...
```{r figure counts human impact Oceania}
...
```{r figure proportions human impact Oceania}
...
# Changes in ecosystem properties constrained by humans 

Considering records where human has had some impact, is there a consistent widespread change in this variation in regions?


Partition for North America

```{r mvpart North America}
...
Partition for Latin America

```{r mvpart Latin America}
...
The ecosystem properties are constrained by spd (and variation by climate and time is removed), hence the variation in CAP1 will mimick the spd predictor. The adj r2 indicate how good the fit is. The ajusted r2 is classified into human impact, which are summaried in the barplots of number counts and proportions of records with different groups of human impact per ecozone and regions. This can be compared with the hvar. However, the classes was arbitrary set by me, though, thinking if human explain more than for example 10 % of the variation in the data, that is moderate impact, and very high if changes explain more than 50% (but no such cases). 

To look at changes over time related to humans, only records where model fit is above 0.1 is included. Then a matrix of age and records are created per region where values are the CAP1 scores. The goal was to find out if there are consistent change in the patterns caused by humans in the ecosystem properties with time. 

The results show that is not the case, or it is not possible to predict any consistent change that are correlated with human presence (CV Error is high = the model has low prediction power). You get R2 by 1-Error, which indicate the model fit the data, which can sometimes be reasonable.  

The classification trees are automatically produced by the mvpart function and shows which splits are signficant in the each region. North America has no splits. 


The conclusion is the same that humans have had some impact on ecosystem properties, but the impact is not widespread, it is localised and not possible to predict. Therefore  based on our results, the hypothesis that past human presence already led to widespread alteration of fundamental ecosystem properties is not the case. Past climate and time is more important all over. Support generally observations that human impact in the past was very localised, as already documented across individual studies of past vegetation changes. 


Question is whether we want to add build some of this in the H1? Is this more informative in terms of the temporal analysis with changes in time?    
