#' @title Run one reusable H1 control profile
#' @description
#' Reuse the canonical temporal control, spatial control, and spatial
#' aggregation operations for one profile-defined human predictor variant.
...
#' @param predictor_vars Named H1 predictor groups or a regional resolver.
#' @param predictor_columns Complete predictor columns required while
#'   collapsing duplicated ages.
#' @param analysis_config Canonical H1 fitting configuration.
#' @param data_profiles Enabled SPD-radius analysis profiles.
#' @param human_proxy Human-proxy key used to select the three profiles.
#' @param configuration_reference Optional configuration-reference filter.
#' @param tag_values Named scalar values appended to every result table.
...
#' )
#' }
run_h1_control_profile <- function(
  ...
  assertthat::assert_that(
    ...
    is.character(predictor_columns),
    is.list(analysis_config),
    is.data.frame(data_profiles),
    assertthat::is.string(human_proxy),
    is.null(configuration_reference) ||
      assertthat::is.string(configuration_reference),
    is.list(tag_values),
    msg = "H1 control profile inputs do not satisfy the contract."
  )
  ...
  profiles_radius <-
    ...
    dplyr::filter(
      ...
      .data[["human_proxy"]] == .env[["human_proxy"]],
      .data[["profile_role"]] == "sensitivity",
      .data[["enabled"]]
    )
  if (!is.null(configuration_reference)) {
    profiles_radius <-
      profiles_radius |>
      dplyr::filter(
        .data[["configuration_reference"]] ==
          .env[["configuration_reference"]]
      )
  }
  ...
  if (
    ...
    cli::cli_abort(
      c(
        "Each H1 control scenario requires three enabled analysis profiles.",
        "i" = paste0(
          "proxy=", human_proxy,
          ", radius=", radius_specification,
          ", configuration=", configuration_reference,
          ", profiles=", nrow(profiles_radius),
          ", controls=", paste(
            profiles_radius[["structural_control"]],
            collapse = ";"
          )
  ...
  tag_values_only <-
    function(data_source) {
      tagged <- data_source
      for (tag_name in rev(names(tag_values))) {
        tagged <-
          tagged |>
          dplyr::mutate(
            !!tag_name := tag_values[[tag_name]],
            .before = 1L
          )
      }
  ...
  output_age_collapse <-
    aggregate_hvar_dataset_ages(
      ...
      predictor_vars = predictor_columns
    )
  ...
  result_time_control <-
    summarise_temporal_hvarpart_results(
      ...
      analysis = stringr::str_c("spatial_", human_proxy)
    )
  ...
  data_timebins <-
    prepare_hvarpart_timebin_data(
      data_source = data_hvar_unique_age |>
        dplyr::select(-dplyr::all_of("region")),
      data_meta = data_meta
    ) |>
  ...
  output_spatial_control <-
    fit_spatial_hvarpart_dataset(
      ...
      analysis = stringr::str_c("temporal_", human_proxy),
      response_vars = response_vars,
      predictor_vars = predictor_vars,
      permutations = analysis_config[["permutations"]],
      alpha = analysis_config[["alpha"]],
