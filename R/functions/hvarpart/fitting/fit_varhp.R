...
#'   partitioning. Defaults to `fit_rdacca_hierarchical_partitioning`.
...
fit_varhp <- function(data_source,
                      response_dist = NULL,
  ...
  if (is.null(hvar_fun)) {
    assertthat::assert_that(
      exists("fit_rdacca_hierarchical_partitioning", mode = "function"),
      msg = "`fit_rdacca_hierarchical_partitioning` must be available or provide `hvar_fun`."
    )
    hvar_fun <-
      get("fit_rdacca_hierarchical_partitioning", mode = "function")
  }

  if (is.null(perm_fun)) {
    assertthat::assert_that(
      exists("compute_hvarpart_permutation_significance", mode = "function"),
      msg = "`compute_hvarpart_permutation_significance` must be available or provide `perm_fun`."
    )
    perm_fun <-
      get("compute_hvarpart_permutation_significance", mode = "function")
  }
