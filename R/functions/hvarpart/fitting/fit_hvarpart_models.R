#' @title Fit hierarchial variation partitioning
...
#' `fit_varhp()` error. If `FALSE`, store `NULL` for failed groups and continue.
#' @return
#' A data frame with original columns and a `varhp` list-column containing
#' outputs from `fit_varhp()`.

fit_hvarpart_models <- function(data_source,
                         response_vars = c(
                           "n0",
                           "n1",
                           "n2",
