#' @title Compute sample-level pollen counts with spatial and temporal bins
#' @description
...
#' )
#' }
compute_pollen_sample_counts <- function(data_source,
                                         stage_label,
                                         time_step = 500) {
  required_cols <-
    c(
  ...
  res_table <-
    ...
    dplyr::mutate(
      ...
      data_rowsums = purrr::map(
        .x = counts_harmonised,
        .f = ~ summarise_pollen_sample_counts(data_counts = .x)
      ),
