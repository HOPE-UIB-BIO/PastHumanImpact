...
#' )
#' }
plot_hvarpart_spatial_signed <- function(
  data_importance,
  data_meta,
  data_geo_koppen
) {
  ...
  signed_fill_scale <- function(guide = "none") {
    result <-
      ggplot2::scale_fill_gradientn(
        "Signed human allocation",
        colours = signed_palette,
        values = c(0, 0.5, 1),
        limits = c(0, 1),
        oob = scales::squish,
        breaks = c(0, 0.5, 1),
  ...
  summarise_signed <- function(group_vars) {
    result <-
  ...
  data_climatezone_summary <-
    summarise_signed(c("analysis", "region", "climatezone")) |>
    add_climatezone_as_factor() |>
    dplyr::mutate(
      summary_colour = signed_colour(.data[["pooled_allocation"]])
    )
  data_region_summary <-
    summarise_signed(c("analysis", "region")) |>
    dplyr::mutate(
      line_colour = signed_colour(.data[["pooled_allocation"]])
    )
