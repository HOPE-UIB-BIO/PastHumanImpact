#' @title Plot Issue 325 spatial sensitivity results
#' @description
#' Build the supplementary Moran, thinning, leave-out, HVarPart comparison,
#' and pure spatial-fraction plots from exported Issue 325 tables.
...
#' @param data_partial Region-by-age pure partial fractions.
...
plot_spatial_sensitivity_results <- function(
  ...
  data_baseline <-
    data_sensitivity |>
    dplyr::filter(
      .data[["sensitivity_type"]] == "baseline",
      .data[["aggregation_level"]] == "overall"
    )
  data_adjusted <-
    data_estimates |>
  ...
  res_plots <-
    list(
      moran = plot_moran,
      thinning = plot_thinning,
      leave_out = plot_leave_out,
      temporal_hvarpart = plot_temporal,
      temporal_partial = plot_partial
    )
