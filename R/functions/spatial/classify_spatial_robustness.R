...
#' @description
#' Apply the predeclared Issue 325 thresholds to overall spatially filtered,
...
#' @param data_spatial_estimates Estimates from `analyse_spatial_importance()`.
    ...
    purrr::map(
      .f = ~ {
        ...
        data_thinning <-
          ...
          dplyr::summarise(
            agreement = mean(.data[["ranking"]] == baseline_ranking),
            .groups = "drop"
          )
        ...
        deterministic_agreement <-
          ...
          all(data_deterministic[["ranking"]] == baseline_ranking)
        spatial_agreement <-
          length(spatial_ranking) == 1L &&
          !is.na(spatial_ranking) &&
          spatial_ranking == baseline_ranking
