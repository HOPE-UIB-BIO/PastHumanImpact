...
#' @description
#' Recalculate signed and zero-truncated importance balances for the baseline,
...
#' @param data_thinning Retention ledger from `run_spatial_thinning()`.
#' @return
#' A common-schema table with balances, rankings, sample counts, and deviations
#' from the matching unthinned baseline.
#' @examples
#' \dontrun{
#' summarise_spatial_importance_sensitivity(
#'   data_records = figure2_records,
...
summarise_spatial_importance_sensitivity <- function(
  ...
  data_reference <-
    ...
    dplyr::transmute(
      ...
      baseline_balance = .data[["importance_balance"]]
    ) |>
    dplyr::mutate(
  ...
  data_sensitivity <-
    ...
    dplyr::mutate(
      absolute_deviation = abs(
        .data[["importance_balance"]] - .data[["baseline_balance"]]
      )
    )
