...
-   The delay has been due to tackling technical issues (code breaking, finding "bugs", package updates, etc).
-   The HGAM models of the density variables - keep breaking - running for most sites but there are issues due to some error in the smoother. I have not at this stage have the time to break this up and manage to find a global k to run it all at once.
-   include the uncertainty matrix of ages and random select ages for each run (as discussed and agree). I need some help to find a good way to structure this in the workflow.
-   Need to fix adjust and fix pending issues e.g. age limits for region needs to be adjusted (now end of interest is 12000 for all)
